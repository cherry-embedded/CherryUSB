...
#define CONFIG_USB_DWC2_RXALL_FIFO_SIZE (512 / 4) // #HACK 1024/4 —> 512/4
#endif
...
uint8_t g_usb_ep_num[CONFIG_USBDEV_MAX_BUS] = {
	 [0] = 6 // #TASK Port.A support 4 endpoint
	,[1] = 4 // #TASK Port.B support 6 endpoint
};

#define CONFIG_DEV_EP_NUM g_usb_ep_num[busid]
...
static inline void dwc2_set_mode(uint8_t busid, uint8_t mode)
{
	...
	{
		USB_OTG_GLB->GUSBCFG |= USB_OTG_GUSBCFG_FDMOD; //# 1<<30
	}
...
static void dwc2_set_turnaroundtime(uint8_t busid, uint32_t hclk, uint8_t speed)
{
  ...
  latency to the Data FIFO */
	if (speed == USB_OTG_SPEED_FULL) {
		...
		USB_LOG_INFO("Line :%d, speed is HS.\r\n", __LINE__);
	}
	else {
...
int usb_dc_init(uint8_t busid)
{
	...
	*/
	...
	USB_OTG_GLB->GCCFG = usbd_get_dwc2_gccfg_conf(USBD_BASE); /// 需要根据芯片类型配置
	...
	USB_OTG_DEV->DCFG |= usbd_get_speed_config(busid, fsphy_type, hsphy_type);
...
void USBD_IRQHandler(uint8_t busid)
{
	...
	if ((USB_OTG_GLB->GINTSTS & 0x1U) == USB_OTG_MODE_DEVICE) {
		...
		if (gint_status & USB_OTG_GINTSTS_USBRST) {
			...
			for (uint8_t i = 0U; i < CONFIG_DEV_EP_NUM; i++) {
				if (i == 0U) {
		...
		if (gint_status & USB_OTG_GINTSTS_PXFR_INCOMPISOOUT) {
			...
			for (ep_idx = 1; ep_idx < CONFIG_DEV_EP_NUM; ep_idx++) {
