...
USB_NOCACHE_RAM_SECTION struct usbd_core_priv {
    ...
    usb_osal_mq_t usbd_tx_mq[USB_EP_IN_NUM];
    usb_osal_thread_t usbd_tx_thread[USB_EP_IN_NUM];
#endif
#if defined(CONFIG_USBDEV_RX_THREAD)
    usb_osal_mq_t usbd_rx_mq[USB_EP_OUT_NUM];
    usb_osal_thread_t usbd_rx_thread[USB_EP_OUT_NUM];
#endif
 ...
 */
static bool usbd_set_endpoint(uint8_t busid, const struct usb_endpoint_descriptor *ep_desc)
{
    USB_LOG_INFO("Open Ep:0x%02x Attr:0x%02x Mps:%u\r\n",
                 ep_desc->bEndpointAddress, ep_desc->bmAttributes, ep_desc->wMaxPacketSize);

    return usbd_ep_open(busid, ep_desc) == 0 ? true : false;
}
/**
 ...
 */
static bool usbd_reset_endpoint(uint8_t busid, const struct usb_endpoint_descriptor *ep_desc)
{
    USB_LOG_INFO("Close Ep:0x%02x Attr:0x%02x\r\n",
                 ep_desc->bEndpointAddress, ep_desc->bmAttributes);

    return usbd_ep_close(busid, ep_desc->bEndpointAddress) == 0 ? true : false;
}
...
static bool usbd_get_descriptor(uint8_t busid, uint16_t type_index, uint8_t **data, uint32_t *len)
{
    ...
    switch (type) {
        case USB_DESCRIPTOR_TYPE_DEVICE:
            *data = (uint8_t *)g_usbd_core[busid].descriptors->device_descriptor;
            *len = g_usbd_core[busid].descriptors->device_descriptor[0];
            break;
        case USB_DESCRIPTOR_TYPE_CONFIGURATION:
            if (g_usbd_core[busid].speed == USB_SPEED_HIGH) {
                if (g_usbd_core[busid].descriptors->hs_config_descriptor) {
                    *data = (uint8_t *)g_usbd_core[busid].descriptors->hs_config_descriptor;
                    *len = (g_usbd_core[busid].descriptors->hs_config_descriptor[CONF_DESC_wTotalLength] |
                            (g_usbd_core[busid].descriptors->hs_config_descriptor[CONF_DESC_wTotalLength + 1] << 8));
                } else {
                    found = false;
                }
            } else {
                if (g_usbd_core[busid].descriptors->fs_config_descriptor) {
                    *data = (uint8_t *)g_usbd_core[busid].descriptors->fs_config_descriptor;
                    *len = (g_usbd_core[busid].descriptors->fs_config_descriptor[CONF_DESC_wTotalLength] |
                            (g_usbd_core[busid].descriptors->fs_config_descriptor[CONF_DESC_wTotalLength + 1] << 8));
                } else {
                    found = false;
                }
            }
            ...
            if (index == USB_STRING_LANGID_INDEX) {
                ...
                *len = 4;
                ...
                if (g_usbd_core[busid].descriptors->msosv1_descriptor) {
                    ...
                    *data = g_usbd_core[busid].descriptors->msosv1_descriptor->string;
                    *len = g_usbd_core[busid].descriptors->msosv1_descriptor->string_len;
                } else {
                }
            } else {
                if (g_usbd_core[busid].descriptors->string_descriptor[index - 1]) {
                    str_len = strlen((const char *)g_usbd_core[busid].descriptors->string_descriptor[index - 1]);

                    (*data)[0] = str_len * 2 + 2;
                    (*data)[1] = 0x03;
                    for (uint16_t i = 0; i < str_len; i++) {
                        (*data)[i * 2 + 2] = g_usbd_core[busid].descriptors->string_descriptor[index - 1][i];
                        (*data)[i * 2 + 3] = 0;
                    }
        ...
        case USB_DESCRIPTOR_TYPE_DEVICE_QUALIFIER:
            if (g_usbd_core[busid].descriptors->device_quality_descriptor) {
                *data = (uint8_t *)g_usbd_core[busid].descriptors->device_quality_descriptor;
                *len = g_usbd_core[busid].descriptors->device_quality_descriptor[0];
            } else {
                found = false;
            }
        ...
        case USB_DESCRIPTOR_TYPE_OTHER_SPEED:
            if (g_usbd_core[busid].speed == USB_SPEED_HIGH) {
                if (g_usbd_core[busid].descriptors->fs_other_speed_descriptor) {
                    *data = (uint8_t *)g_usbd_core[busid].descriptors->fs_other_speed_descriptor;
                    *len = (g_usbd_core[busid].descriptors->fs_other_speed_descriptor[CONF_DESC_wTotalLength] |
                            (g_usbd_core[busid].descriptors->fs_other_speed_descriptor[CONF_DESC_wTotalLength] << 8));
                } else {
                    found = false;
                }
            } else {
                if (g_usbd_core[busid].descriptors->hs_other_speed_descriptor) {
                    *data = (uint8_t *)g_usbd_core[busid].descriptors->hs_other_speed_descriptor;
                    *len = (g_usbd_core[busid].descriptors->hs_other_speed_descriptor[CONF_DESC_wTotalLength] |
                            (g_usbd_core[busid].descriptors->hs_other_speed_descriptor[CONF_DESC_wTotalLength] << 8));
                } else {
                    found = false;
                }
            }
    ...
    if (found == false) {
        /* nothing found */
        USB_LOG_ERR("descriptor <type:%02x,index:%02x> not found!\r\n", type, index);
    }
    return found;
}
#else
static bool usbd_get_descriptor(uint8_t busid, uint16_t type_index, uint8_t **data, uint32_t *len)
{
    ...
    if ((type == USB_DESCRIPTOR_TYPE_STRING) && (index == USB_OSDESC_STRING_DESC_INDEX)) {
        ...
        memcpy(*data, (uint8_t *)g_usbd_core[busid].msosv1_desc->string, g_usbd_core[busid].msosv1_desc->string_len);
        *len = g_usbd_core[busid].msosv1_desc->string_len;
...
#ifdef CONFIG_USBDEV_ADVANCE_DESC
    if (g_usbd_core[busid].speed == USB_SPEED_HIGH) {
        p = (uint8_t *)g_usbd_core[busid].descriptors->hs_config_descriptor;
    } else {
        p = (uint8_t *)g_usbd_core[busid].descriptors->fs_config_descriptor;
    }
#else
    p = (uint8_t *)g_usbd_core[busid].descriptors;
#endif
    ...
    switch (setup->bRequest) {
        ...
        case USB_REQUEST_SET_FEATURE:
            if (value == USB_FEATURE_REMOTE_WAKEUP) {
                if (setup->bRequest == USB_REQUEST_SET_FEATURE) {
                    usbd_event_handler(busid, USBD_EVENT_SET_REMOTE_WAKEUP);
                } else {
                    usbd_event_handler(busid, USBD_EVENT_CLR_REMOTE_WAKEUP);
                }
        ...
        case USB_REQUEST_SET_CONFIGURATION:
            ...
            if (!usbd_set_configuration(busid, value, 0)) {
                ...
                usbd_event_handler(busid, USBD_EVENT_CONFIGURED);
            }
            *len = 0;
            break;
 ...
 * @return true if the request was handled successfully
 */
static bool usbd_std_interface_req_handler(uint8_t busid, struct usb_setup_packet *setup,
    ...
    switch (setup->bRequest) {
        ...
        case USB_REQUEST_CLEAR_FEATURE:
            if (setup->wValue == USB_FEATURE_ENDPOINT_HALT) {
                USB_LOG_ERR("ep:0x%02x clear halt\r\n", ep);
        ...
        case USB_REQUEST_SET_FEATURE:
            if (setup->wValue == USB_FEATURE_ENDPOINT_HALT) {
                USB_LOG_ERR("ep:0x%02x set halt\r\n", ep);
        ...
        if (setup->bRequest == g_usbd_core[busid].descriptors->msosv1_descriptor->vendor_code) {
            switch (setup->wIndex) {
                case 0x04:
                    ...
                    *len = g_usbd_core[busid].descriptors->msosv1_descriptor->compat_id_len;
                    return 0;
                case 0x05:
                    USB_LOG_INFO("get Compat id properties\r\n");
                    *data = (uint8_t *)g_usbd_core[busid].descriptors->msosv1_descriptor->comp_id_property;
                    *len = g_usbd_core[busid].descriptors->msosv1_descriptor->comp_id_property_len;
                    return 0;
                default:
        ...
        if (setup->bRequest == g_usbd_core[busid].descriptors->webusb_url_descriptor->vendor_code) {
            switch (setup->wIndex) {
                case WINUSB_REQUEST_GET_DESCRIPTOR_SET:
                    ...
                    *len = g_usbd_core[busid].descriptors->webusb_url_descriptor->string_len;
                    return 0;
                default:
                    USB_LOG_ERR("unknown vendor code\r\n");
                    return -1;
        ...
        if (setup->bRequest == g_usbd_core[busid].msosv1_desc->vendor_code) {
            switch (setup->wIndex) {
                case 0x04:
                    ...
                    memcpy(*data, g_usbd_core[busid].msosv1_desc->compat_id, g_usbd_core[busid].msosv1_desc->compat_id_len);
                    *len = g_usbd_core[busid].msosv1_desc->compat_id_len;

                    return 0;
                case 0x05:
                    USB_LOG_INFO("get Compat id properties\r\n");
                    //*data = (uint8_t *)g_usbd_core[busid].msosv1_desc->comp_id_property;
                    memcpy(*data, g_usbd_core[busid].msosv1_desc->comp_id_property, g_usbd_core[busid].msosv1_desc->comp_id_property_len);
                    *len = g_usbd_core[busid].msosv1_desc->comp_id_property_len;
...
#ifdef CONFIG_USBDEV_ADVANCE_DESC
                if (g_usbd_core[busid].speed == USB_SPEED_FULL) {
...
{
    usbd_event_handler(busid, USBD_EVENT_CONNECTED);
}

void usbd_event_disconnect_handler(uint8_t busid)
{
    usbd_event_handler(busid, USBD_EVENT_DISCONNECTED);
}

void usbd_event_resume_handler(uint8_t busid)
{
    usbd_event_handler(busid, USBD_EVENT_RESUME);
}

void usbd_event_suspend_handler(uint8_t busid)
{
    usbd_event_handler(busid, USBD_EVENT_SUSPEND);
}
...
void usbd_event_reset_handler(uint8_t busid)
{
    ...
    ep_desc.wMaxPacketSize = USB_CTRL_EP_MPS;
    ep_desc.bmAttributes = USB_ENDPOINT_TYPE_CONTROL;
    ep_desc.bEndpointAddress = USB_CONTROL_IN_EP0;
    usbd_ep_open(busid, &ep_desc);

    ep_desc.bEndpointAddress = USB_CONTROL_OUT_EP0;
    usbd_ep_open(busid, &ep_desc);

    usbd_class_event_notify_handler(busid, USBD_EVENT_RESET, NULL);
    usbd_event_handler(busid, USBD_EVENT_RESET);
}
...
#endif
#ifdef CONFIG_USBDEV_ADVANCE_DESC
    if ((uint32_t)g_usbd_core[busid].ep0_data_buf & (CONFIG_USB_ALIGN_SIZE - 1)) {
        USB_LOG_ERR("ep0_data_buf do not align with %d\r\n", CONFIG_USB_ALIGN_SIZE);
        usbd_ep_set_stall(busid, USB_CONTROL_IN_EP0);
        return;
    }
#endif
...
#else
    g_usbd_core[busid].tx_msg[ep & 0x7f].nbytes = nbytes;
    usb_osal_mq_send(g_usbd_core[busid].usbd_tx_mq[ep & 0x7f], (uintptr_t)&g_usbd_core[busid].tx_msg[ep & 0x7f]);
#endif
}
...
#else
    g_usbd_core[busid].rx_msg[ep & 0x7f].nbytes = nbytes;
    usb_osal_mq_send(g_usbd_core[busid].usbd_rx_mq[ep & 0x7f], (uintptr_t)&g_usbd_core[busid].rx_msg[ep & 0x7f]);
#endif
}
...
static void usbdev_tx_rx_thread(void *argument)
{
    ...
    int ret;
    usb_osal_mq_t mq = (usb_osal_mq_t)argument;

    while (1) {
        ret = usb_osal_mq_recv(mq, (uintptr_t *)&msg, 0xffffffff);
        if (ret < 0) {
            continue;
        }
...
void usbd_add_endpoint(uint8_t busid, struct usbd_endpoint *ep)
{
    if (ep->ep_addr & 0x80) {
        ...
        if (g_usbd_core[busid].usbd_tx_mq[ep & 0x7f] == NULL) {
            g_usbd_core[busid].usbd_tx_mq[ep & 0x7f] = usb_osal_mq_create(1);
            if (g_usbd_core[busid].usbd_tx_mq[ep & 0x7f] == NULL) {
                while (1) {
                }
            }
        }
        if (g_usbd_core[busid].usbd_tx_thread[ep->ep_addr & 0x7f] == NULL) {
            g_usbd_core[busid].usbd_tx_thread[ep->ep_addr & 0x7f] = usb_osal_thread_create("usbd_tx",
                                                                                           CONFIG_USBDEV_TX_STACKSIZE,
                                                                                           CONFIG_USBDEV_TX_PRIO,
                                                                                           usbdev_tx_rx_thread,
                                                                                           g_usbd_core[busid].usbd_tx_mq[ep & 0x7f]);
            if (g_usbd_core[busid].usbd_tx_thread[ep->ep_addr & 0x7f] == NULL) {
                while (1) {
                }
            }
        ...
        if (g_usbd_core[busid].usbd_rx_mq[ep & 0x7f] == NULL) {
            g_usbd_core[busid].usbd_rx_mq[ep & 0x7f] = usb_osal_mq_create(1);
            if (g_usbd_core[busid].usbd_rx_mq[ep & 0x7f] == NULL) {
                while (1) {
                }
            }
        }
        if (g_usbd_core[busid].usbd_rx_thread[ep->ep_addr & 0x7f] == NULL) {
            g_usbd_core[busid].usbd_rx_thread[ep->ep_addr & 0x7f] = usb_osal_thread_create("usbd_rx",
                                                                                           CONFIG_USBDEV_RX_STACKSIZE,
                                                                                           CONFIG_USBDEV_RX_PRIO,
                                                                                           usbdev_tx_rx_thread,
                                                                                           g_usbd_core[busid].usbd_rx_mq[ep & 0x7f]);
            if (g_usbd_core[busid].usbd_rx_thread[ep->ep_addr & 0x7f] == NULL) {
                while (1) {
                }
            }
...
{
    return usbd_udc_init(busid);
}

int usbd_deinitialize(uint8_t busid)
{
    g_usbd_core[busid].intf_offset = 0;
    usbd_udc_deinit(busid);
#if defined(CONFIG_USBDEV_TX_THREAD)
#endif
#if defined(CONFIG_USBDEV_RX_THREAD)
#endif
