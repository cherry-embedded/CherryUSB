...
extern UINT8  Usb2DevEndp0Size;              // USB设备的端点0的最大包尺寸 */
...
extern const UINT8  SetupGetU2DevDescr[];    // 获取设备描述符*/
extern const UINT8  SetupGetU2CfgDescr[];    // 获取配置描述符*/
extern const UINT8  SetupSetUsb2Addr[];     // 设置USB地址*/
extern const UINT8  SetupSetUsb2Config[];   // 设置USB配置*/
extern const UINT8  SetupSetUsb2Interface[];// 设置USB接口配置*/
extern const UINT8  SetupClrU2EndpStall[];   // 清除端点STALL*/
...
UINT8   AnalyzeRootU2Hub( void ) ;         // 分析ROOT-U2HUB状态,处理ROOT-U2HUB端口的设备插拔事件
// 返回ERR_SUCCESS为没有情况,返回ERR_USB_CONNECT为检测到新连接,返回ERR_USB_DISCON为检测到断开
void    SetHostUsb2Addr( UINT8 addr );                 // 设置USB主机当前操作的USB设备地址
void    SetUsb2Speed( UINT8 FullSpeed );               // 设置当前USB速度
void    ResetRootU2HubPort(void);                          // 检测到设备后,复位相应端口的总线,为枚举设备准备,设置为默认为全速
UINT8   EnableRootU2HubPort(void);                          // 使能ROOT-HUB端口,相应的bUH_PORT_EN置1开启端口,设备断开可能导致返回失败
void    SelectU2HubPort( UINT8 HubPortIndex );// HubPortIndex=0选择操作指定的ROOT-HUB端口,否则选择操作指定的ROOT-HUB端口的外部HUB的指定端口
UINT8   WaitUSB2_Interrupt( void );                    // 等待USB中断
// 传输事务,输入目的端点地址/PID令牌,同步标志,以20uS为单位的NAK重试总时间(0则不重试,0xFFFF无限重试),返回0成功,超时/出错重试
UINT8   USB2HostTransact( UINT8 endp_pid, UINT8 tog, UINT32 timeout );  // endp_pid: 高4位是token_pid令牌, 低4位是端点地址
UINT8   U2HostCtrlTransfer( PUINT8 DataBuf, PUINT8 RetLen );  // 执行控制传输,8字节请求码在pSetupReq中,DataBuf为可选的收发缓冲区
// 如果需要接收和发送数据,那么DataBuf需指向有效缓冲区用于存放后续数据,实际成功收发的总长度返回保存在ReqLen指向的字节变量中


void CopyU2SetupReqPkg( PCCHAR pReqPkt );            // 复制控制传输的请求包
UINT8 CtrlGetU2DeviceDescr( void );                    // 获取设备描述符,返回在 pHOST_TX_RAM_Addr 中
UINT8 CtrlGetU2ConfigDescr( void );                    // 获取配置描述符,返回在 pHOST_TX_RAM_Addr 中
UINT8 CtrlSetUsb2Address( UINT8 addr );                         // 设置USB设备地址
UINT8 CtrlSetUsb2Config( UINT8 cfg );                           // 设置USB设备配置
UINT8 CtrlClearU2EndpStall( UINT8 endp ) ;                       // 清除端点STALL
UINT8 CtrlSetUsb2Intercace( UINT8 cfg );                // 设置USB设备接口
...
UINT8   CtrlGetU2HIDDeviceReport( UINT8 infc );          // HID类命令，SET_IDLE和GET_REPORT
UINT8   CtrlGetU2HubDescr( void );                       // 获取HUB描述符,返回在TxBuffer中
UINT8   U2HubGetPortStatus( UINT8 HubPortIndex );        // 查询HUB端口状态,返回在TxBuffer中
UINT8   U2HubSetPortFeature( UINT8 HubPortIndex, UINT8 FeatureSelt );  // 设置HUB端口特性
UINT8   U2HubClearPortFeature( UINT8 HubPortIndex, UINT8 FeatureSelt );  // 清除HUB端口特性
