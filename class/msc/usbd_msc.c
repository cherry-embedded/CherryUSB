...
static bool SCSI_readFormatCapacity(uint8_t busid, uint8_t **data, uint32_t *len)
{
    ...
    uint8_t format_capacity[SCSIRESP_READFORMATCAPACITIES_SIZEOF] = {
        ...
        (uint8_t)((g_usbd_msc[busid].scsi_blk_nbr >> 24) & 0xff),
        (uint8_t)((g_usbd_msc[busid].scsi_blk_nbr >> 16) & 0xff),
        (uint8_t)((g_usbd_msc[busid].scsi_blk_nbr >> 8) & 0xff),
        (uint8_t)((g_usbd_msc[busid].scsi_blk_nbr >> 0) & 0xff),

        0x02, /* Descriptor Code: Formatted Media */
        0x00,
        (uint8_t)((g_usbd_msc[busid].scsi_blk_size >> 8) & 0xff),
        (uint8_t)((g_usbd_msc[busid].scsi_blk_size >> 0) & 0xff),
    };
...
static bool SCSI_readCapacity10(uint8_t busid, uint8_t **data, uint32_t *len)
{
    ...
    uint8_t capacity10[SCSIRESP_READCAPACITY10_SIZEOF] = {
        (uint8_t)(((g_usbd_msc[busid].scsi_blk_nbr - 1) >> 24) & 0xff),
        (uint8_t)(((g_usbd_msc[busid].scsi_blk_nbr - 1) >> 16) & 0xff),
        (uint8_t)(((g_usbd_msc[busid].scsi_blk_nbr - 1) >> 8) & 0xff),
        (uint8_t)(((g_usbd_msc[busid].scsi_blk_nbr - 1) >> 0) & 0xff),

        (uint8_t)((g_usbd_msc[busid].scsi_blk_size >> 24) & 0xff),
        (uint8_t)((g_usbd_msc[busid].scsi_blk_size >> 16) & 0xff),
        (uint8_t)((g_usbd_msc[busid].scsi_blk_size >> 8) & 0xff),
        (uint8_t)((g_usbd_msc[busid].scsi_blk_size >> 0) & 0xff),
    };
...
static bool SCSI_read10(uint8_t busid, uint8_t **data, uint32_t *len)
{
    ...
    if (g_usbd_msc[busid].cbw.dDataLength != (g_usbd_msc[busid].nsectors * g_usbd_msc[busid].scsi_blk_size)) {
...
static bool SCSI_write10(uint8_t busid, uint8_t **data, uint32_t *len)
{
    ...
    data_len = g_usbd_msc[busid].nsectors * g_usbd_msc[busid].scsi_blk_size;
    if ((g_usbd_msc[busid].start_sector + g_usbd_msc[busid].nsectors) > g_usbd_msc[busid].scsi_blk_nbr) {
        USB_LOG_ERR("LBA out of range\r\n");
        return false;
    }
...
static bool SCSI_verify10(uint8_t **data, uint32_t *len)
{
    ...
    g_usbd_msc[busid].scsi_blk_addr = lba * g_usbd_msc[busid].scsi_blk_size;

    /* Number of Blocks to transfer */
    blk_num = GET_BE16(&g_usbd_msc[busid].cbw.CB[7]);

    USB_LOG_DBG("num (block) : 0x%x\r\n", blk_num);
    g_usbd_msc[busid].scsi_blk_len = blk_num * g_usbd_msc[busid].scsi_blk_size;

    if ((lba + blk_num) > g_usbd_msc[busid].scsi_blk_nbr) {
        USB_LOG_ERR("LBA out of range\r\n");
        return false;
    }
...
static bool SCSI_processRead(uint8_t busid)
{
    ...
    transfer_len = MIN(g_usbd_msc[busid].nsectors * g_usbd_msc[busid].scsi_blk_size, CONFIG_USBDEV_MSC_BLOCK_SIZE);

    if (usbd_msc_sector_read(busid, g_usbd_msc[busid].start_sector, g_usbd_msc[busid].block_buffer, transfer_len) != 0) {
    ...
    g_usbd_msc[busid].start_sector += (transfer_len / g_usbd_msc[busid].scsi_blk_size);
    g_usbd_msc[busid].nsectors -= (transfer_len / g_usbd_msc[busid].scsi_blk_size);
...
static bool SCSI_processWrite(uint8_t busid, uint32_t nbytes)
{
    ...
    if (usbd_msc_sector_write(busid, g_usbd_msc[busid].start_sector, g_usbd_msc[busid].block_buffer, nbytes) != 0) {
        SCSI_SetSenseData(busid, SCSI_KCQHE_WRITEFAULT);
        return false;
    }

    g_usbd_msc[busid].start_sector += (nbytes / g_usbd_msc[busid].scsi_blk_size);
    g_usbd_msc[busid].nsectors -= (nbytes / g_usbd_msc[busid].scsi_blk_size);
    g_usbd_msc[busid].csw.dDataResidue -= nbytes;

    if (g_usbd_msc[busid].nsectors == 0) {
        usbd_msc_send_csw(busid, CSW_STATUS_CMD_PASSED);
    } else {
        data_len = MIN(g_usbd_msc[busid].nsectors * g_usbd_msc[busid].scsi_blk_size, CONFIG_USBDEV_MSC_BLOCK_SIZE);
        usbd_ep_start_read(busid, mass_ep_data[busid][MSD_OUT_EP_IDX].ep_addr, g_usbd_msc[busid].block_buffer, data_len);
    }
...
static bool SCSI_CBWDecode(uint8_t busid, uint32_t nbytes)
{
    ...
    if ((g_usbd_msc[busid].cbw.bLUN > 1) || (g_usbd_msc[busid].cbw.dSignature != MSC_CBW_Signature) || (g_usbd_msc[busid].cbw.bCBLength < 1) || (g_usbd_msc[busid].cbw.bCBLength > 16)) {
        SCSI_SetSenseData(busid, SCSI_KCQIR_INVALIDCOMMAND);
        return false;
    } else {
...
struct usbd_interface *usbd_msc_init_intf(uint8_t busid, struct usbd_interface *intf, const uint8_t out_ep, const uint8_t in_ep)
{
    ...
    if (g_usbd_msc[busid].scsi_blk_size > CONFIG_USBDEV_MSC_BLOCK_SIZE) {
